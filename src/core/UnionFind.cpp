...
// N-1: Iterative find with path compression and safety limit
int UnionFind::find(int x) {
    ...
    while (steps++ < safetyLimit) {
        int p;
        if (inDenseRange(root)) {
            p = parent_vec[root - idOffset];
        } else {
            if (!parent.count(root)) return -1;
            p = parent[root];
        }
        if (p == root) break;
        root = p;
    ...
    while (x != root) {
        int next;
        if (inDenseRange(x)) {
            next = parent_vec[x - idOffset];
            parent_vec[x - idOffset] = root;
        } else {
            next = parent[x];
            parent[x] = root;
        }
