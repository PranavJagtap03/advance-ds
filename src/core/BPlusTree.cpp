...
// 14. removeHelper — C-3: recursive underflow propagation, O-2: correct internal minKeys
void BPlusTree::removeHelper(BPlusNode* node, BPlusNode* parent, int childIndex, string key, bool& underflow) {
    if (!node) return;

    // O-2: Separate thresholds for leaf vs internal underflow
    const int leafMin = (ORDER - 1) / 2;          // = 1 for ORDER=4
    const int internalMin = (ORDER - 1 + 1) / 2;  // = 2 for ORDER=4 (ceiling)
    ...
    if (node->isLeaf) {
        ...
        if (node != root && (int)node->keys.size() < leafMin) {
            underflow = true;
        }
        return;
    }
    ...
    // O-2: Use appropriate min based on whether child is leaf or internal
    int childMin = child->isLeaf ? leafMin : internalMin;
    ...
    // O-2: Propagate underflow upward using internalMin for internal nodes
    if (node != root && (int)node->keys.size() < internalMin) {
        underflow = true;
    }
}

// 15. remove(key) — C-3: with root collapse, R-3: targeted sizeIndex removal
void BPlusTree::remove(string key) {
    if (!root) return;

    // R-3: Record existing node's size before deletion for targeted sizeIndex erase
    long existingSize = -1;
    FileNode* existing = search(key);
    ...
    // R-3: Targeted sizeIndex removal using lower_bound
    if (existingSize != -1) {
        auto lo = lower_bound(sizeIndex.begin(), sizeIndex.end(),
                              make_pair(existingSize, key));
        if (lo != sizeIndex.end() && lo->first == existingSize && lo->second == key) {
            sizeIndex.erase(lo);
        }
    }
}
